...
// Bun.write cria os diretórios pai sozinho, então basta o caminho final.
const DATA_DIR = process.env.YELLOWTV_DATA_DIR ?? `${process.cwd()}/data`;
const OUTPUT_FILE = `${DATA_DIR}/lista_pro.json`;
...
function parseChannels(
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (isMediaFile(urlLine)) continue;
...
async function main() {
    ...
    for (let i = 0; i < M3U_URLS.length; i++) {
        ...
        try {
            ...
            console.log(
                `   ↳ ${channels.length} canais únicos obtidos desta fonte (${seenNames.size} total acumulado).\n`,
            );
    ...
    // Bun.write cria o diretório automaticamente e retorna os bytes escritos
    const bytes = await Bun.write(OUTPUT_FILE, JSON.stringify(channels, null, 2));
    ...
    console.log(`\n💾 Salvo em: ${OUTPUT_FILE} (${(bytes / 1024).toFixed(1)} KB)`);
}
